...
return new class extends Migration
{
    ...
    {
        if (DB::table('sales_commission_hold_adjustments')->where('adjustment_kind', 'late_legal_entity_entitlement')->exists()) {
            throw new RuntimeException('Rollback refused: export and reconcile late legal-entity commission entitlement evidence first.');
        }
        if (DB::getDriverName() === 'pgsql') {
            DB::statement('ALTER TABLE sales_commission_hold_adjustments DROP CONSTRAINT IF EXISTS sales_commission_hold_adjustment_kind');
