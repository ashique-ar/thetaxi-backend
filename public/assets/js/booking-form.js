...
(function () {
    ...
    window.initializeGoogleMaps = function () {
        ...
        initializeLocationSearch();
        console.log("Google Maps initialized successfully (for location search only)");
    };
    ...
    function setupCustomTour() {
        ...
        const addButtonBottom = document.getElementById("add-destination-bottom");
        if (addButtonBottom) {
            addButtonBottom.addEventListener("click", function () {
                ...
                setTimeout(() => {
                    ...
                    if (lastDestination) {
                        lastDestination.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    }
                }, 100);
            });
        }
        ...
        div.innerHTML = `
            <div class="destination-item-header">
                <div class="d-flex align-items-center">
                    ...
                    <h6 class="destination-title mb-0">Destination ${index + 1}</h6>
                </div>
    ...
    function initializeLeafletRouteMap(mapElement) {
        ...
        state.customTourDestinations.forEach((dest, index) => {
            ...
            const marker = L.marker([dest.lat, dest.lng], { icon: markerIcon }).addTo(map);
            
            // Create popup content
            const popupContent = `
                <div class="route-marker-popup">
                    <h6>${isStart ? 'Starting Point' : isEnd ? 'Final Destination' : `Stop ${index}`}</h6>
                    <p><strong>${dest.name}</strong></p>
                    ${dest.date ? `<p>📅 ${dest.date} ${dest.time || ''}</p>` : ''}
    ...
    function addDirectionArrows(coordinates) {
        for (let i = 0; i < coordinates.length - 1; i++) {
            ...
            const arrowIcon = L.divIcon({
                ...
                html: `<div style="transform: rotate(${angle + 90}deg);">→</div>`,
                iconSize: [20, 20],
                iconAnchor: [10, 10]
            });
            ...
            item.innerHTML = `
                <div class="modal-destination-card">
                    <div class="modal-destination-header">
                        ...
                        <span class="badge ${
                            isStart ? "bg-success" : isEnd ? "bg-danger" : "bg-primary"
                        }">${index + 1}</span>
                    </div>
                    ...
                    <div class="modal-destination-form mt-2">
                        <div class="mb-2">
                            ...
                            <input type="text" class="form-control form-control-sm location-input-modal" 
                                   data-index="${index}" value="${dest.name}" placeholder="Enter location">
                        </div>
                        ...
                        <div class="row mb-2">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control form-control-sm date-input-modal" 
                                       data-index="${index}" value="${dest.date || ''}" placeholder="Select date">
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="mb-2">
                            ...
                            <textarea class="form-control form-control-sm notes-input-modal" 
                                      data-index="${index}" rows="2" placeholder="Add notes...">${dest.notes || ''}</textarea>
                        </div>
                        ` : ''}
    ...
    function createDestinationElementFromData(destData, index) {
        ...
        div.innerHTML = `
            ...
            <div class="destination-fields">
                <div class="destination-location">
                    <div class="single-search-box location-search-box">
                        ...
                        <div class="custom-select-dropdown">
                            <input type="text" name="destinations[${index}][location]" placeholder="Destination Location" class="location-search" value="${destData.name}" required>
                ...
                <div class="destination-datetime">
                    <div class="single-search-box date-field">
                        ...
                        <div class="custom-select-dropdown">
                            <input type="text" name="destinations[${index}][visit_date]" placeholder="Visit Date" class="custom-datepicker" value="${destData.date || ''}" required>
                        </div>
                    </div>
                    <div class="single-search-box">
                        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="custom-select-dropdown">
                            <input type="time" name="destinations[${index}][visit_time]" value="${destData.time || '09:00'}" required>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="destination-notes">
                <textarea name="destinations[${index}][notes]" placeholder="Special notes for this destination (optional)" rows="2">${destData.notes || ''}</textarea>
            </div>
        `;

        // Add remove functionality
        div.querySelector(".destination-remove").addEventListener("click", function () {
            div.remove();
            // Remove from state
    ...
    function validateDate(input) {
        ...
        if (selectedDate < today) {
            showDateError(input, 'Past dates are not allowed. Please select today or a future date.');
    ...
    function showDateError(input, message) {
        const $input = $(input);
        const $parent = $input.closest('.single-search-box, .destination-datetime');
        ...
        // Add error message
        const $error = $('<div class="date-error-message"></div>').text(message);
        $parent.append($error);
    }
    ...
    function setupFormAnimations() {
        ...
        searchBoxes.forEach(box => {
            ...
            box.addEventListener('mouseleave', function() {
                if (!this.classList.contains('error') && !this.matches(':focus-within')) {
