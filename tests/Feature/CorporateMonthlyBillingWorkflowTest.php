...
beforeEach(function () {
    ...
    Schema::create('bookings', function(Blueprint $t){$t->uuid('id')->primary();$t->string('booking_number')->nullable();$t->boolean('is_corporate_booking')->default(true);$t->uuid('corporate_account_id');$t->string('status')->nullable();$t->string('currency',3)->default('LKR');$t->decimal('total_estimated',12,2)->nullable();$t->decimal('total_actual',12,2)->nullable();$t->timestamps();$t->softDeletes();});
...
it('versions terms previews finalized trips and generates one immutable monthly statement', function () {
    ...
    DB::table('bookings')->insert([['id'=>'30000000-0000-4000-8000-000000000001','booking_number'=>'A-1','corporate_account_id'=>$corporateId,'status'=>'completed','currency'=>'LKR','total_actual'=>150],['id'=>'30000000-0000-4000-8000-000000000002','booking_number'=>'O-1','corporate_account_id'=>$otherId,'status'=>'completed','currency'=>'LKR','total_actual'=>900]]);
    DB::table('booking_items')->insert([['id'=>'40000000-0000-4000-8000-000000000001','booking_id'=>'30000000-0000-4000-8000-000000000001','service_type_id'=>'20000000-0000-4000-8000-000000000001','from_date'=>'2026-08-10','to_date'=>'2026-08-10','status'=>'completed','final_priced_at'=>'2026-08-10 12:00:00','total_price'=>150,'currency'=>'LKR','pricing_breakdown'=>'{"final_pricing":{"total_amount":150}}'],['id'=>'40000000-0000-4000-8000-000000000002','booking_id'=>'30000000-0000-4000-8000-000000000001','service_type_id'=>'20000000-0000-4000-8000-000000000001','from_date'=>'2026-08-12','to_date'=>'2026-08-12','status'=>'pending','final_priced_at'=>null,'total_price'=>50,'currency'=>'LKR','pricing_breakdown'=>null],['id'=>'40000000-0000-4000-8000-000000000003','booking_id'=>'30000000-0000-4000-8000-000000000002','service_type_id'=>'20000000-0000-4000-8000-000000000001','from_date'=>'2026-08-11','to_date'=>'2026-08-11','status'=>'completed','final_priced_at'=>'2026-08-11 12:00:00','total_price'=>900,'currency'=>'LKR','pricing_breakdown'=>null]]);
    ...
    expect($preview['booking_count'])->toBe(1)->and($preview['trip_count'])->toBe(1)->and($preview['charges_total'])->toBe(150.0)->and($preview['excluded'])->toHaveCount(1)->and($preview['excluded'][0]['reason'])->toBe('not_completed');
