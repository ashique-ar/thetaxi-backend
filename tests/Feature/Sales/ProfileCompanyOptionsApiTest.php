...
it('searches and hydrates only companies in the actors effective Profile scope', function () {
    ...
    SalesProfile::query()->create([
        ...
        'sales_code' => 'PROFILE-001', 'status' => 'active', 'effective_from' => now()->subDay(),
        'staff_category_snapshot' => 'sales', 'reporting_currency' => 'LKR', 'acquisition_eligible' => true,
    ]);
    $foreign = Company::create(['name' => 'Foreign Profile Company']);
    $deleted = Company::create(['name' => 'Deleted Profile Company']); $deleted->delete();
    $url = '/api/sales/profile-company-options';
