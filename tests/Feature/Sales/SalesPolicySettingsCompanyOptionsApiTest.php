...
it('searches and hydrates only non-deleted companies belonging to an active Staff identity', function () {
    [$admin, $company] = hr_seed_admin_actor(['name' => 'Scoped Policy Company', 'city' => 'Colombo']);
    ...
    $actor->user->givePermissionTo('sales.policy-settings.view');
    $foreign = Company::create(['name' => 'Foreign Policy Company']);
    $url = '/api/sales/policy-settings/company-options';

    $response = actingAs($actor->user, 'api')->getJson($url.'?search=Scoped&per_page=1')->assertOk()
        ->assertJsonPath('data.data.0.value', $company->id)->assertJsonPath('data.data.0.label', 'Scoped Policy Company');
