...
it('uses an authorized bounded booking selector for SMS previews', function () {
    ...
    $sendAction = substr($controller, strpos($controller, 'public function send('), strpos($controller, 'public function sendTest(') - strpos($controller, 'public function send('));
    ...
    expect($controller)->toContain("'per_page' => ['nullable', 'integer', 'min:1', 'max:50']", "'booking_id' => ['nullable', 'uuid']", "can('bookings.view')", 'previewBooking(', "'value' => (string) \$booking->id")
        ...
        ->and($sendAction)->not->toContain("'context_type'", "'context_id'")
        ->and($smsService)->not->toContain("'context_type' => \$payload['context_type']", "'context_id' => \$payload['context_id']");
});
