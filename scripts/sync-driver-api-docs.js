...
const onboardingFolder = {
    ...
    item: [
        ...
        requestItem('Save Vehicle Step', 'PATCH', '{{base_url}}/api/driver/onboarding/steps/4', { make_id: 'make-uuid', model_id: 'model-uuid', model_year: 2024, color: 'White', registration_year: 2024, license_plate: 'CAB-1234', is_owner: true }, 'Send make_id from List Vehicle Makes and model_id from List Models by Make. The model must belong to the selected make. Approval classifies the vehicle group.', [response('OK', 200, { status: 'success', data: { id: 'application-uuid', status: 'draft', current_step: 5, payload: { vehicle: { make_id: 'make-uuid', model_id: 'model-uuid' } } } })]),
...
const vehicleStep = { type: 'object', additionalProperties: false, required: ['make_id', 'model_id', 'model_year', 'color', 'registration_year', 'license_plate', 'is_owner'], properties: { make_id: { type: 'string', format: 'uuid', description: 'ID from GET /api/driver/onboarding/makes.' }, model_id: { type: 'string', format: 'uuid', description: 'ID from GET /api/driver/onboarding/makes/{make_id}/models; must belong to make_id.' }, model_year: { type: 'integer', minimum: 1950 }, color: { type: 'string', maxLength: 50 }, registration_year: { type: 'integer', minimum: 1950 }, license_plate: { type: 'string', maxLength: 30 }, is_owner: { type: 'boolean' } } };
openapi.paths['/api/driver/onboarding/steps/{step}'] = { patch: { tags: ['Driver Onboarding'], summary: 'Save identity, address, or vehicle onboarding step', description: 'Payload must match the selected step: 1 identity, 3 address, 4 vehicle. Address country_id and state_id are backend UUIDs returned by the lookup endpoints, and the state must belong to the selected country.', security: protectedSecurity, parameters: [{ name: 'step', in: 'path', required: true, schema: { type: 'integer', enum: [1, 3, 4] } }], requestBody: { required: true, content: { 'application/json': { schema: { oneOf: [identityStep, addressStep, vehicleStep] }, examples: { identity: { value: { first_name: 'Nimal', last_name: 'Perera', email: 'nimal@example.com', nic: '901234567V' } }, address: { value: { address: '10 Main Street', country_id: 'country-uuid', state_id: 'state-uuid', city: 'Colombo', postal_code: '00100' } }, vehicle: { value: { make_id: 'make-uuid', model_id: 'model-uuid', model_year: 2024, color: 'White', registration_year: 2024, license_plate: 'CAB-1234', is_owner: true } } } } } }, responses: { ...onboardingResponse, 403: errorResponse('Only reviewer-listed fields may be changed'), 409: errorResponse('Application cannot be edited') } } };
