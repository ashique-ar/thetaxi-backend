...
class CorporateWorkspaceController extends Controller
{
    public function dashboard(Request $request): JsonResponse
    {
        ...
        $all = $user->can('view_all_bookings');
        ...
        return response()->json(['status' => 'success', 'data' => [
            'role_mode' => $user->can('manage_employees') ? 'administrator' : ($user->can('view_payments') ? 'finance' : ($user->can('approve_bookings') ? 'approver' : ($all ? 'coordinator' : 'employee'))),
            'scope' => $all ? 'company' : 'employee',
            'tasks' => $tasks,
            'capabilities' => collect(['create_bookings', 'create_bookings_for_others', 'view_all_bookings', 'approve_bookings', 'view_payments', 'view_reports', 'manage_employees', 'view_audit_log'])->mapWithKeys(fn ($permission) => [$permission => $user->can($permission)]),
        ]]);
    }
}
