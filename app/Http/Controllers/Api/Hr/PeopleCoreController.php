...
class PeopleCoreController extends Controller
{
    ...
    public function putStaffCustomFieldValue(Request $request,string $staffId,string $definitionId):JsonResponse
    {
        ...
        $staff=Staff::withTrashed()->findOrFail($staffId);$this->access->authorize($request->user(),$staff);
    ...
    {
        $this->ensureEnabled();$companyId=$this->access->actorCompanyId($request->user());
        $data=$request->validate($this->organizationUnitRules(true));$this->assertOrganizationStaffReferences($data,$companyId);
    ...
    {
        return [
            'code'=>['required','string','max:80'],'name'=>['required','string','max:255'],
    ...
    private function documentTypeRules(bool $update=false):array
    {
        return [
            ...
            'required_for_staff_types'=>['nullable','array'],'required_for_staff_types.*'=>['string','max:80'],
            'required_for_employment_types'=>['nullable','array'],'required_for_employment_types.*'=>['string','max:80'],
            'requires_expiry'=>['required','boolean'],'renewal_reminder_days'=>['nullable','integer','min:0','max:365'],
            'status'=>$update?['required',Rule::in(['active','inactive'])]:['prohibited'],
            'effective_from'=>['required','date'],'effective_until'=>['nullable','date','after:effective_from'],
            'reason'=>['required','string','max:2000'],'idempotency_key'=>['required','string','max:160'],
            ...($update?['expected_version'=>['required','integer','min:1']]:[]),
    ...
    {
        $this->ensureEnabled();$companyId=$this->access->actorCompanyId($request->user());$data=$request->validate($this->customFieldRules());$this->assertCustomFieldRules($data);
    ...
    {
        $this->ensureEnabled();$companyId=$this->access->actorCompanyId($request->user());$data=$request->validate($this->customFieldRules(true));$this->assertCustomFieldRules($data);
    ...
    public function timeline(Request $request, string $staffId): JsonResponse
    {
        ...
        if(!$request->user()->can('hr.people.timeline-confidential')) $query->whereIn('confidentiality',['employee','manager','internal']);
    ...
    {
        $this->ensureEnabled();
        $staff=Staff::withTrashed()->findOrFail($case->staff_id);$this->access->authorize($request->user(),$staff);
        ...
        foreach (['employment_type_id'=>'hr_employment_types','position_id'=>'hr_positions','organization_unit_id'=>'hr_organization_units'] as $field=>$table) {
            if (! empty($assignment[$field])) abort_unless(DB::table($table)->where('id',$assignment[$field])->where('company_id',$case->company_id)->exists(),422,"{$field} must belong to the employee legal entity.");
        }
        if (! empty($assignment['manager_staff_id'])) abort_unless(Staff::query()->whereKey($assignment['manager_staff_id'])->where('company_id',$case->company_id)->exists(),422,'The manager must belong to the employee legal entity.');
    ...
    public function profileVersions(Request $request,string $staffId):JsonResponse
    {
        ...
        if($request->user()->can('hr.people.timeline-confidential'))$rows->each(fn($row)=>$row->makeVisible('encrypted_profile'));
        return response()->json(['status'=>'success','data'=>$rows]);
    }
    ...
    public function storeProfileVersion(Request $request,string $staffId):JsonResponse
    {
        ...
        $row=$this->people->addProfileVersion($staff,$data['profile'],$data['change_reason'],(string)$request->user()->id);$row->makeVisible('encrypted_profile');
    ...
    public function records(Request $request,string $staffId):JsonResponse
    {
        ...
        if($request->user()->can('hr.people.timeline-confidential'))$page->getCollection()->each(fn($row)=>$row->makeVisible('encrypted_data'));
        return response()->json(['status'=>'success','data'=>$page]);
    }
    ...
    public function storeRecord(Request $request,string $staffId):JsonResponse
    {
        ...
        if (! empty($data['employment_spell_id'])) abort_unless(HrEmploymentSpell::query()->whereKey($data['employment_spell_id'])->where('staff_id',$staff->id)->exists(),422,'The employment spell does not belong to this employee.');
        $payload=$data; $payload['encrypted_data']=$payload['data'];unset($payload['data']);$payload['verification_status']='unverified';
        $row=$this->people->addEmployeeRecord($staff,$payload,(string)$request->user()->id);$row->makeVisible('encrypted_data');
        return response()->json(['status'=>'success','data'=>$row],201);
    }
    ...
    private function organizationUnitRules(bool $update=false):array
    {
        return [
            ...
            'code'=>['required','string','max:80'],'name'=>['required','string','max:255'],
            'manager_staff_id'=>['nullable','uuid','exists:staff,id'],'hr_partner_staff_id'=>['nullable','uuid','exists:staff,id'],
            'timezone'=>['required','timezone'],'status'=>$update?['required',Rule::in(['active','inactive'])]:['prohibited'],
            'effective_from'=>['required','date'],'effective_until'=>['nullable','date','after:effective_from'],'custom_fields'=>['prohibited'],
            'reason'=>['required','string','max:2000'],'idempotency_key'=>['required','string','max:160'],
            ...($update?['expected_version'=>['required','integer','min:1']]:[]),
        ];
    }
    ...
    private function customFieldRules(bool $update=false):array
    {
        return [
            ...
            'field_key'=>['required','alpha_dash','max:80'],'label'=>['required','string','max:255'],
            'data_type'=>['required',Rule::in(['text','long_text','integer','decimal','boolean','date','datetime','select','multi_select','email','phone','url'])],
            'validation_rules'=>['nullable','array:options,min_length,max_length,minimum,maximum'],'validation_rules.options'=>['nullable','array','max:200'],'validation_rules.options.*'=>['string','max:255'],
            'validation_rules.min_length'=>['nullable','integer','min:0','max:10000'],'validation_rules.max_length'=>['nullable','integer','min:1','max:10000'],
            'validation_rules.minimum'=>['nullable','string','max:100','regex:/^-?(0|[1-9][0-9]*)(\.[0-9]{1,8})?$/'],'validation_rules.maximum'=>['nullable','string','max:100','regex:/^-?(0|[1-9][0-9]*)(\.[0-9]{1,8})?$/'],
            'confidentiality'=>['required',Rule::in(['employee','manager','internal','hr_private','legal'])],
            'required'=>['required','boolean'],'active'=>[$update?'required':'nullable','boolean'],
            'reason'=>['required','string','max:2000'],'idempotency_key'=>['required','string','max:160'],
            ...($update?['expected_version'=>['required','integer','min:1']]:[]),
        ];
    }
    ...
    {
        foreach(['manager_staff_id','hr_partner_staff_id']as$field)if(!empty($data[$field]))abort_unless(Staff::query()->whereKey($data[$field])->where('company_id',$companyId)->exists(),422,"{$field} must belong to the organization legal entity.");
    }

    private function assertCustomFieldRules(array $data):void
    {
        $options=$data['validation_rules']['options']??[];$select=in_array($data['data_type'],['select','multi_select'],true);
        ...
        if(isset($rules['minimum'],$rules['maximum']))abort_if($this->compareExactDecimals($rules['minimum'],$rules['maximum'])>0,422,'Custom-field minimum cannot exceed maximum.');
    }

    private function compareExactDecimals(string $left,string $right):int
    {
        $normalize=function(string$value):array{$negative=str_starts_with($value,'-');$unsigned=$negative?substr($value,1):$value;[$integer,$fraction]=array_pad(explode('.',$unsigned,2),2,'');$integer=ltrim($integer,'0')?:'0';$fraction=rtrim($fraction,'0');if($integer==='0'&&$fraction==='')$negative=false;return[$negative,$integer,$fraction];};
        [$ln,$li,$lf]=$normalize($left);[$rn,$ri,$rf]=$normalize($right);if($ln!==$rn)return$ln?-1:1;$m=strlen($li)<=>strlen($ri);if($m===0)$m=strcmp($li,$ri)<=>0;if($m===0){$length=max(strlen($lf),strlen($rf));$m=strcmp(str_pad($lf,$length,'0'),str_pad($rf,$length,'0'))<=>0;}return$ln?-$m:$m;
    }

    private function decodeJsonColumns(object $row,array $columns):array
    {
        $data=(array)$row;foreach($columns as$column)if(isset($data[$column])&&is_string($data[$column]))$data[$column]=json_decode($data[$column],true,512,JSON_THROW_ON_ERROR);return$data;
    }
    ...
    private function employeeSummary(Staff $staff): array
    {
        ...
        return [
            'id'=>$staff->id,'employee_number'=>$staff->code,'name'=>trim(($staff->user?->first_name??'').' '.($staff->user?->last_name??'')),
            'email'=>$staff->user?->email,'phone'=>$staff->user?->phone,'company_id'=>$staff->company_id,'company_name'=>$staff->company?->name,
            'staff_category'=>$staff->staff_type,'employment_status'=>$currentSpell?->status??($staff->employment_ended_at||$staff->trashed()?'former':'incomplete'),
            'employment_ended_at'=>$staff->employment_ended_at,'current_spell_id'=>$currentSpell?->id,
            ...
            'profile_complete'=>$issues===[],'profile_issues'=>$issues,
            'sales_category_configured'=>$categories->isNotEmpty(),'is_sales_category'=>$categories->contains(mb_strtolower(trim((string)$staff->staff_type))),
