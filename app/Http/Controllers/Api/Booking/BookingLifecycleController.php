...
class BookingLifecycleController extends Controller
{
        ...
        try {
            ...
            $dispatch = $this->lifecycleService->dispatchVehicle(
                ...
                array_merge([
                    ...
                    'allow_repeat_dispatch_for_testing' => $request->boolean('allow_repeat_dispatch_for_testing'),
                ], [
    ...
    public function processReturn(Request $request): JsonResponse
    {
        $request->validate([
            ...
            'pickup_waiting_minutes' => 'nullable|integer|min:0',
            'hire_waiting_minutes' => 'nullable|integer|min:0',
    ...
    public function completeQCInspection(Request $request): JsonResponse
    {
        $request->validate([
            ...
            'damages_found' => 'nullable|array',
            'damages_found.*.location' => 'required_with:damages_found|string|max:255',
            'damages_found.*.description' => 'required_with:damages_found|string',
            'damages_found.*.severity' => 'required_with:damages_found|in:minor,moderate,major',
            'issues_reported' => 'nullable|array',
            'issues_reported.*.category' => 'required_with:issues_reported|string|max:255',
            'issues_reported.*.description' => 'required_with:issues_reported|string',
            'issues_reported.*.severity' => 'required_with:issues_reported|in:low,medium,high',
            'issues_reported.*.action_required' => 'nullable|string',
