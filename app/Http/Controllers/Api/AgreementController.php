...
class AgreementController extends Controller
{
    ...
    public function reports(): JsonResponse { return response()->json(['stats' => $this->statsData(), 'items' => Agreement::latest()->get()]); }
    ...
    public function bulkExport(Request $request): Response
    {
        ...
        if ($data['format']==='excel') {
            $csv="Title,Type,Status,Start Date,End Date\n".$agreements->map(fn($a)=>collect([$a->title,$a->type,$a->status,$a->start_date?->format('Y-m-d'),$a->end_date?->format('Y-m-d')])->map(fn($v)=>'"'.str_replace('"','""',(string)$v).'"')->implode(','))->implode("\n");
            return response($csv,200,['Content-Type'=>'text/csv','Content-Disposition'=>'attachment; filename="agreements.csv"']);
        }
    ...
    {
        $lines=[]; foreach($agreements as $a){$lines[]=$a->title; $lines[]='Status: '.$a->status.'  Type: '.($a->type ?: 'N/A'); $lines[]='Dates: '.($a->start_date?->format('Y-m-d') ?: 'N/A').' - '.($a->end_date?->format('Y-m-d') ?: 'Open'); if($a->signed_at)$lines[]='Signed at: '.$a->signed_at->format('Y-m-d H:i'); if(data_get($a->metadata,'identity_verified'))$lines[]='Identity verified: '.data_get($a->metadata,'identity_verification_method','yes'); foreach($a->signatures ?? [] as $signature){$lines[]='Signature: '.($signature['signature_method'] ?? 'typed').' at '.($signature['signed_at'] ?? 'N/A');} if($a->description)$lines[]=$a->description; $lines[]='';}
