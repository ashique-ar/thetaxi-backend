...
class SalesPerformanceService
{
    ...
    public const RANKING_POLICY = [
        ...
        'missing_target' => 'n/a_after_configured_targets',
        'ties' => 'equal_display_rank',
    ...
    ): SalesTargetVersion
    {
        return DB::transaction(function () use ($data, $idempotencyKey, $actorUserId, $sourceIp) {
            ...
            if ($existing) {
                abort_unless(
                    hash_equals((string) $existing->draft_request_checksum, $requestChecksum),
                    409,
                ...
                $outboxEventId = DB::table('domain_outbox_events')
                    ...
                    ->where('correlation_id', $idempotencyKey)->value('id');
                abort_unless(
                    is_string($auditEventId) && is_string($outboxEventId),
                    409,
            ...
            $payload = [
                ...
                'version' => $version,
                'reason' => trim($data['reason']),
                'draft_idempotency_key' => $idempotencyKey,
                'draft_request_checksum' => $requestChecksum,
            ...
            DB::table('domain_audit_events')->insert([
                ...
                'domain' => 'sales',
                'company_id' => $profile->company_id,
                'subject_type' => 'sales_target_version',
                'subject_id' => $target->id,
                'event_type' => 'sales.performance.target_draft_created',
                'actor_user_id' => $actorUserId,
                'actor_type' => 'user',
                'correlation_id' => $idempotencyKey,
                'source_ip' => $sourceIp,
                'before_checksum' => null,
                'after_checksum' => $target->payload_checksum,
                'reason' => trim($data['reason']),
                'occurred_at' => $target->prepared_at,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
            $outboxEventId = $this->events->record(
                'sales',
                $profile->company_id,
                'sales_target_version',
                $target->id,
                'sales.performance.target_draft_created',
                1,
                1,
                [
                    'sales_profile_id' => $profile->id,
    ...
    ): SalesTargetVersion
    {
        return DB::transaction(function () use ($target, $expectedVersion, $reason, $idempotencyKey, $actorUserId, $sourceIp, ) {
            ...
            if ($replay) {
                abort_unless(
                    $replay->id === $locked->id
                    && hash_equals((string) $replay->approval_request_checksum, $requestChecksum),
                    409,
            ...
            $timezone = $this->policySettings->businessTimezone((string) $locked->company_id);
            abort_unless(
            ...
            abort_if(
                DB::table('domain_period_locks')->where('domain', 'sales')->where('company_id', $locked->company_id)
                    ->where('period_type', 'month')->where('period_start', $startUtc)->where('period_end', $endExclusiveUtc)
                    ->where('state', 'locked')->exists(),
                409,
            ...
            DB::table('domain_audit_events')->insert([
                ...
                'event_type' => 'sales.performance.target_approved',
                'actor_user_id' => $actorUserId,
                'actor_type' => 'user',
                'correlation_id' => $idempotencyKey,
                'source_ip' => $sourceIp,
            ...
            $outboxEventId = $this->events->record(
                ...
                'sales_target_version',
                $locked->id,
                'sales.performance.target_approved',
                2,
                1,
                [
    ...
    ): array
    {
        ...
        abort_unless(
            $profiles->count() === count($profileIds),
            422,
            'Every selected Sales Profile must belong to the selected legal entity.'
        );

        $rows = collect($profileIds)->map(function (string $profileId) use ($profiles, $sourceStart, $sourceEnd, $targetStart, $targetEnd) {
    ...
    ): SalesTargetCopyBatch
    {
        ...
        return DB::transaction(function () use ($companyId, $sourcePeriodStart, $targetPeriodStart, $profileIds, $previewChecksum, $reason, $idempotencyKey, $actorUserId, $requestChecksum) {
            ...
            if ($existing) {
                abort_unless(
                    hash_equals($existing->request_payload_checksum, $requestChecksum),
                    409,
            ...
            abort_unless(
                hash_equals($preview['preview_checksum'], $previewChecksum),
                409,
            ...
            $batch = SalesTargetCopyBatch::create([
                ...
                'target_period_start' => $targetStart->toDateString(),
                'target_period_end' => $targetEnd->toDateString(),
                'selection_snapshot' => $preview['rows'],
                'preview_checksum' => $previewChecksum,
                'reason' => trim($reason),
                'prepared_by' => $actorUserId,
                'prepared_at' => now(),
            ...
            foreach ($preview['rows'] as $row) {
                ...
                $payload = [
                    ...
                    'copy_batch_id' => $batch->id,
                    'copied_from_target_id' => $row['source_target_id'],
                    'period_start' => $targetStart->toDateString(),
                    'period_end' => $targetEnd->toDateString(),
            ...
            $this->events->record(
                'sales',
                $companyId,
                'sales_target_copy_batch',
                $batch->id,
                'sales.performance.targets_copied_to_draft',
                1,
                1,
                [
                    'source_period_start' => $sourceStart->toDateString(),
                    'target_period_start' => $targetStart->toDateString(),
                    'copied_count' => $preview['copyable_count'],
    ...
    ): array
    {
        ...
        abort_unless(
            is_string($fileChecksum) && strlen($fileChecksum) === 64,
            422,
        ...
        $rows = collect($parsed)->map(function (array $row) use ($profiles, $duplicateCodes, $targetStart, $targetEnd): array {
            $profile = $profiles->get($row['sales_code']);
            $errors = $row['errors'];
            if ($row['sales_code'] !== '' && $duplicateCodes->has($row['sales_code'])) {
                $errors[] = [
                    'field_name' => 'sales_code',
                    'error_code' => 'duplicate_sales_code',
            ...
            if ($row['sales_code'] !== '' && !$profile) {
                $errors[] = [
                    'field_name' => 'sales_code',
                    'error_code' => 'profile_not_found_or_outside_scope',
            ...
            if ($profile && !$profileEffective) {
                $errors[] = [
                    'field_name' => 'sales_code',
                    'error_code' => 'profile_not_effective',
            ...
            return [
                ...
                'destination_latest_version' => $destination?->version,
                'destination_latest_status' => $destination?->status,
                'action' => $errors === [] ? 'create_draft' : 'blocked',
                'errors' => $errors,
    ...
    ): object
    {
        ...
        $requestChecksum = hash('sha256', CanonicalJson::encode([
            'company_id' => $companyId,
            'target_period_start' => $preview['target_period_start'],
        ...
        try {
            return DB::transaction(function () use ($companyId, $targetPeriodStart, $file, $authorizedProfileIds, $preview, $previewChecksum, $reason, $idempotencyKey, $actorUserId, $requestChecksum, &$storedPath): object {
                ...
                if ($existing) {
                    abort_unless(
                        hash_equals((string) $existing->request_payload_checksum, $requestChecksum),
                        409,
                        'The target-import idempotency key was reused with different evidence.'
                    );
                    if ($authorizedProfileIds !== null) {
                        abort_if(
                            SalesTargetVersion::query()->where('import_job_id', $existing->id)
                                ->whereNotIn('sales_profile_id', $authorizedProfileIds)->exists(),
                            403,
                ...
                $this->events->record(
                    ...
                    'domain_transfer_job',
                    $jobId,
                    'sales.performance.targets_imported_to_draft',
                    1,
                    1,
                    [
                        'target_period_start' => $targetStart->toDateString(),
            ...
            if ($storedPath)
                Storage::disk('sales_private')->delete($storedPath);
            throw $exception;
        }
    }
        ...
        try {
            ...
            while (($values = fgetcsv($handle, null, ',', '"', '')) !== false) {
                ...
                if (count($values) !== 3) {
                    $errors[] = [
                        'field_name' => null,
                        'error_code' => 'column_count_invalid',
                ...
                if ($salesCode === '') {
                    $errors[] = [
                        'field_name' => 'sales_code',
                        'error_code' => 'sales_code_required',
                        'message' => 'Sales code is required.'
                    ];
                } elseif (mb_strlen($salesCode) > 80) {
                    $errors[] = [
                        'field_name' => 'sales_code',
                        'error_code' => 'sales_code_too_long',
                ...
                $rows[] = [
                    ...
                    'eligible_collections_target_lkr' => $collections,
                    'errors' => $errors,
                ];
            }
    ...
    ): SalesPerformanceAlert
    {
        return DB::transaction(function () use ($alert, $action, $expectedVersion, $reason, $key, $actorUserId, $snoozedUntil) {
            ...
            if ($replay) {
                abort_unless(
                    $replay->alert_id === $locked->id && hash_equals($replay->request_payload_checksum, $checksum),
                    409,
                    'Alert action idempotency key was already used for a different request.'
                );
                return $locked;
            }

            abort_unless(
                $this->policySettings->featureEnabled((string) $locked->company_id, 'performance_alert_actions'),
                409,
                'Sales performance alert actions are not activated for this legal entity.'
            );
            ...
            $this->events->record(
                'sales',
                $locked->company_id,
                'sales_performance_alert',
                $locked->id,
                'sales.performance.alert_' . $action,
                $toVersion,
                1,
                [
                    'from_status' => $fromStatus,
                    ...
                    'from_version' => $expectedVersion,
                    'to_version' => $toVersion,
                    'action_event_checksum' => $checksum,
                    'snoozed_until' => $snoozeAt?->toIso8601String(),
                    'escalation_level' => $action === 'escalate' ? $locked->escalation_level : null,
                ],
                $now,
    ...
    private function calculateRows(string $companyId, string $from, string $to, $cutoff, ?array $alertRules = null): Collection
    {
        ...
        $breakdowns = $this->metricBreakdowns->resolve(
            $collectionFacts->flatten(1)->concat($commissionFacts->flatten(1))->values(),
            $companyId,
            $from,
            $to,
        );
        ...
        $rows = $profiles->map(function (SalesProfile $profile) use ($facts, $collectionFacts, $commissionFacts, $breakdowns, $targets, $adjustmentEvidenceIssues, $overdue, $taskFacts, $from, $to, $overdueAsOf, $agingByProfile, $agingSource) {
            ...
            $target = $this->proratedTargets($targets->get($profile->id, collect()), (string) $profile->company_id, $from, $to);
            ...
            return [
                ...
                'new_sales_target_lkr' => $newTarget,
                'collection_target_lkr' => $collectionTarget,
                ...
                'existing_booking_collections_lkr' => $priorBookingCollections,
                'eligible_collections_lkr' => $collectionTotal,
                ...
                ...($taskFacts[$profile->id] ?? [
                    ...
                    'oldest_overdue_task_age_days' => null,
                    'missed_next_action_count' => null,
                    'missed_next_action_lookback_completed_months' => null,
                    'task_deadline_or_history_missing_count' => null,
                    'task_intervention_evidence_checksum' => null,
                ]),
        ...
        return $sorted->map(function ($row, $index) use (&$rank, &$previous) {
            if ($row['new_sales_achievement_percent'] === null) {
    ...
    private function proratedTargets(Collection $targets, string $companyId, string $from, string $to): array
    {
        $businessTimezone = $this->policySettings->businessTimezone($companyId);
        abort_unless(
            is_string($businessTimezone) && in_array($businessTimezone, \DateTimeZone::listIdentifiers(), true),
            409,
            'An approved Sales business timezone is required for target period calculations.'
        );
    ...
    private function assertPeriod(string $from, string $to, string $periodType): void
    {
        ...
        if ($periodType === 'month')
            abort_unless($start->isStartOfMonth() && $end->isSameDay($start->endOfMonth()), 422, 'Monthly snapshots require a complete calendar month.');
        if ($periodType === 'quarter')
            abort_unless($start->month % 3 === 1 && $start->isStartOfMonth() && $end->isSameDay($start->addMonths(2)->endOfMonth()), 422, 'Quarter snapshots require a complete calendar quarter.');
        if ($periodType === 'year')
            abort_unless($start->isStartOfYear() && $end->isSameDay($start->endOfYear()), 422, 'Year snapshots require a complete calendar year.');
    }

    public function alertEvaluationContext(SalesKpiSnapshot $snapshot): array
    {
        abort_unless(
            $this->policySettings->featureEnabled((string) $snapshot->company_id, 'performance_alert_evaluations'),
            409,
            'Sales performance alert evaluation is not activated for this legal entity.'
        );
        abort_unless(
            $snapshot->status === 'frozen' && $snapshot->period_type === 'month',
            409,
        ...
        abort_unless(
            $policy && $policy->approved_at && $policy->approved_at->lte($snapshot->generated_at),
            409,
        ...
        return DB::transaction(function () use ($snapshot): object {
            ...
            foreach ($snapshot->rows as $row) {
                ...
                if ($rules['no_new_sales']['enabled'] && (int) $row->new_bookings_count === 0) {
                    $checks['no_new_sales'] = [
                        $rules['no_new_sales']['severity'],
                        'No factual New Sales were recorded in the completed month.',
                        ['operator' => 'equals', 'quantity' => 0],
                        ['actual_quantity' => (int) $row->new_bookings_count]
                    ];
                }
                if ($rules['no_sales_activity']['enabled'] && (int) $row->activities_count === 0) {
                    $checks['no_sales_activity'] = [
                        $rules['no_sales_activity']['severity'],
                        'No factual Sales activities were recorded in the completed month.',
                        ['operator' => 'equals', 'quantity' => 0],
                        ['actual_quantity' => (int) $row->activities_count]
                    ];
                }
                $overdueRule = $rules['overdue_collections'];
                if (
                    $overdueRule['enabled'] && (float) $row->overdue_collections_lkr >= $overdueRule['minimum_amount_lkr']
                    && (int) ($metrics['oldest_overdue_age_days'] ?? 0) >= $overdueRule['minimum_age_days']
                ) {
                    $checks['overdue_collections'] = [
                        $overdueRule['severity'],
                ...
                if ($overdueTaskRule['enabled']) {
                    if (
                        $taskHistoryMissing > 0 || ($metrics['overdue_task_count'] ?? null) === null
                        || ($metrics['task_intervention_evidence_checksum'] ?? null) === null
                    ) {
                        $suppressed[] = [
                            'sales_profile_id' => $row->sales_profile_id,
                            'rule' => 'overdue_tasks',
                            'reason' => 'governed_task_deadline_or_history_missing',
                            'missing_count' => $taskHistoryMissing
                        ];
                    } elseif (
                        (int) $metrics['overdue_task_count'] >= $overdueTaskRule['minimum_count']
                        && (int) ($metrics['oldest_overdue_task_age_days'] ?? 0) >= $overdueTaskRule['minimum_age_days']
                    ) {
                        $checks['overdue_tasks'] = [
                            $overdueTaskRule['severity'],
                            ...
                            [
                                ...
                                'status_basis' => $overdueTaskRule['status_basis'],
                                'owner_basis' => $overdueTaskRule['owner_basis']
                            ],
                            [
                ...
                if ($missedActionRule['enabled']) {
                    if (
                        $taskHistoryMissing > 0 || ($metrics['missed_next_action_count'] ?? null) === null
                        || ($metrics['task_intervention_evidence_checksum'] ?? null) === null
                    ) {
                        $suppressed[] = [
                            'sales_profile_id' => $row->sales_profile_id,
                        ...
                        $checks['repeatedly_missed_next_actions'] = [
                            $missedActionRule['severity'],
                            ...
                            [
                                ...
                                'owner_basis' => $missedActionRule['owner_basis'],
                                'source' => $missedActionRule['source']
                            ],
                            [
                ...
                if ($relianceRule['enabled']) {
                    if (
                        ($metrics['new_sales_value_state'] ?? 'incomplete') !== 'complete'
                        ...
                        $suppressed[] = [
                            'sales_profile_id' => $row->sales_profile_id,
                            'rule' => 'recurring_commission_reliance',
                            'reason' => 'required_dimension_or_target_missing'
                        ];
                    } elseif (
                        $metrics['prior_booking_commission_ratio_percent'] >= $relianceRule['prior_booking_commission_percent']
                        && (float) $row->new_sales_achievement_percent < $relianceRule['new_sales_achievement_below_percent']
                    ) {
                        $checks['recurring_commission_reliance'] = [
                            $relianceRule['severity'],
                ...
                if ($declineRule['enabled']) {
                    ...
                    if (
                        ...
                        $suppressed[] = [
                            'sales_profile_id' => $row->sales_profile_id,
                            'rule' => 'new_sales_decline',
                            'reason' => 'completed_month_baseline_incomplete',
                            'available_months' => $history->count(),
                        ...
                        if ($decline >= $declineRule['percent']) {
                            $checks['new_sales_decline'] = [
                                $declineRule['severity'],
                                ...
                                [
                                    'actual_decline_percent' => round($decline, 4),
                                    'baseline_average_lkr' => round($average, 4),
                ...
                foreach ($checks as $type => [$severity, $explanation, $threshold, $comparison]) {
                    ...
                    $alert = SalesPerformanceAlert::firstOrCreate(['deduplication_key' => $deduplicationKey], [
                        ...
                        'policy_contract_snapshot' => $rules,
                        'threshold_snapshot' => $threshold,
                        'comparison_snapshot' => $comparison,
                        'evaluation_checksum' => hash('sha256', CanonicalJson::encode($evaluation)),
                        'detected_at' => now(),
                    ]);
                    $alertsCreated++;
                }
            ...
            if (
                $rules['evaluation']['baseline_completeness'] === 'require_complete'
            ...
            $result = [
                ...
                'scheduled_for' => $dueAt->toIso8601String(),
                'profile_count' => $snapshot->rows->count(),
            ...
            $this->events->record(
                'sales',
                $snapshot->company_id,
                'sales_alert_evaluation',
                $runId,
                'sales.performance.alerts_evaluated',
                1,
                1,
                [
                    'snapshot_id' => $snapshot->id,
                ...
                $now,
                "alert-evaluation:{$snapshot->id}:{$policy->id}"
            );
