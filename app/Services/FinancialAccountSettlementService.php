...
class FinancialAccountSettlementService
{
    ...
    public function create(array $data, ?string $userId): FinancialAccountSettlement
    {
        if ($data['owner_type'] === 'corporate') {
            throw ValidationException::withMessages([
                'owner_type' => ['Corporate settlements must be generated from Corporate Billing so completed trips, final prices, and monthly-invoice eligibility are enforced.'],
            ]);
        }
    ...
    public function issue(FinancialAccountSettlement $settlement, array $data): FinancialAccountSettlement
    {
        ...
        if (($data['send_invoice'] ?? true) === true)
            $this->sendDocument($settlement, $document);
    ...
    public function receiveCorporateRemittance(Corporate $corporate, array $data, ?string $userId): CorporateRemittance
    {
        return DB::transaction(function () use ($corporate, $data, $userId) {
            Corporate::query()->whereKey($corporate->id)->lockForUpdate()->firstOrFail();
            $duplicate = CorporateRemittance::where('idempotency_key', $data['idempotency_key'])->first();
            if ($duplicate) {
                abort_unless((string) $duplicate->corporate_id === (string) $corporate->id, 422, 'This payment submission key belongs to another corporate account.');
                return $duplicate->load('allocations.settlement');
            }

            $settlements = FinancialAccountSettlement::query()->where('owner_type', 'corporate')->where('owner_id', $corporate->id)
                ->whereIn('status', ['open', 'partial', 'overdue'])
                ->when($data['settlement_ids'] ?? null, fn($query, $ids) => $query->whereIn('id', $ids))
                ->orderByRaw('due_date IS NULL')->orderBy('due_date')->orderBy('issued_at')->lockForUpdate()->get();
            $currencyMismatch = $settlements->first(fn($settlement) => strtoupper((string) data_get($settlement->billing_terms_snapshot, 'currency', $data['currency'])) !== strtoupper($data['currency']));
            if ($currencyMismatch)
                throw ValidationException::withMessages(['currency' => ['All selected invoices must use the remittance currency.']]);

            $remittance = CorporateRemittance::create([
                ...$data,
    ...
    public function updateCollectionFollowUp(FinancialAccountSettlement $settlement, array $data, ?string $userId): FinancialAccountSettlement
    {
        ...
        $this->audit('account_settlement', $settlement->id, 'collection_follow_up_updated', $settlement->status, $settlement->status, null, $data, $userId);
        return $settlement->fresh(['items.booking', 'document']);
    }
    ...
    public function recalculate(FinancialAccountSettlement $settlement): FinancialAccountSettlement
    {
        ...
        if ($status !== 'draft' && $status !== 'void' && $status !== 'disputed') {
            $status = $outstanding <= 0 ? 'paid' : ($payments > 0 ? 'partial' : 'open');
            if ($outstanding > 0 && $settlement->issued_at && $settlement->due_date?->isPast())
                $status = 'overdue';
        }
        $settlement->update([
            'charges_total' => $items->sum('charge_amount'),
            'payments_total' => $payments,
            'refunds_total' => $items->sum('refund_amount'),
            'adjustments_total' => $items->sum('adjustment_amount'),
    ...
    public function settleDriverCash(array $data, ?string $userId): DriverCashSettlement
    {
        return DB::transaction(function () use ($data, $userId) {
            ...
            if ((float) $data['amount'] > $held)
                throw ValidationException::withMessages(['amount' => ['Handover amount exceeds cash currently held by this driver.']]);
            ...
            foreach ($receipts as $receipt) {
                if ($remaining <= 0)
                    break;
                $open = (float) $receipt->amount - (float) $receipt->driver_company_settled_amount;
    ...
    public function adjust(FinancialAccountSettlement $settlement, array $data, ?string $userId): FinancialAccountSettlement
    {
        ...
        return DB::transaction(function () use ($settlement, $data, $userId) {
            ...
            if (in_array($data['type'], ['refund', 'credit_note'], true))
                ...
                $item->adjustment_amount = (float) $item->adjustment_amount + ($data['type'] === 'additional_charge' ? $amount : -$amount);
            ...
            $item->status = (float) $item->outstanding_amount <= 0 ? 'paid' : 'open';
            $item->save();
            if ($data['type'] === 'waiver')
                $item->booking?->update(['payment_status' => 'waived', 'refund_status' => 'waived']);
    ...
    {
        $from = $settlement->status;
        $settlement->update(['status' => 'disputed', 'dispute_reason' => $reason, 'disputed_at' => now(), 'disputed_by' => $userId]);
    ...
    public function resolveDispute(FinancialAccountSettlement $settlement, string $notes, ?string $userId): FinancialAccountSettlement
    {
        ...
        $settlement = $this->recalculate($settlement);
        $this->audit('account_settlement', $settlement->id, 'dispute_resolved', 'disputed', $settlement->status, null, ['notes' => $notes], $userId);
        return $settlement;
    }
    ...
    public function generateDocument(FinancialAccountSettlement $settlement): FinancialSettlementDocument
    {
        ...
        try {
            $pdf = Pdf::loadView('invoices.financial-settlement', ['settlement' => $settlement, 'ownerName' => $ownerName]);
            $path = 'invoices/settlements/' . $settlement->invoice_number . '.pdf';
            Storage::disk('local')->put($path, $pdf->output());
            $document->update(['pdf_path' => $path, 'generated_at' => now()]);
            $this->audit('account_settlement', $settlement->id, 'invoice_generated', null, $document->status, null, ['invoice_number' => $document->invoice_number, 'document_id' => $document->id], auth()->id());
        } catch (\Throwable $e) {
            $document->update(['last_error' => $e->getMessage()]);
            $this->audit('account_settlement', $settlement->id, 'invoice_generation_failed', null, $settlement->status, null, ['invoice_number' => $document->invoice_number, 'document_id' => $document->id, 'error_class' => get_class($e)], auth()->id());
            throw $e;
        }
    ...
    public function sendDocument(FinancialAccountSettlement $settlement, FinancialSettlementDocument $document): void
    {
        ...
        if ($recipients->isEmpty()) {
            $document->update(['last_error' => 'No billing email is recorded for this account.']);
            $this->audit('account_settlement', $settlement->id, 'invoice_delivery_failed', null, $settlement->status, null, ['invoice_number' => $document->invoice_number, 'document_id' => $document->id, 'reason' => 'billing_email_missing'], auth()->id());
            return;
        }
        ...
        try {
            ...
            $document->update(['last_error' => $e->getMessage()]);
            $this->audit('account_settlement', $settlement->id, 'invoice_delivery_failed', null, $settlement->status, null, ['invoice_number' => $document->invoice_number, 'document_id' => $document->id, 'error_class' => get_class($e)], auth()->id());
        }
    }
