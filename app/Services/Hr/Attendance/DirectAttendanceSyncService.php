...
class DirectAttendanceSyncService
{
    ...
    public function sync(AttendanceDevice $device, CarbonImmutable $from, CarbonImmutable $to, string $type = 'incremental'): array
    {
        ...
        try {
            do {
                ...
                foreach ($page['events'] as $event) $this->persistEvent($device, $runId, $event, $counts);
    ...
    {
        if (AttendanceRawEvent::query()->where('device_id',$device->id)->where('provider_event_id',$event['provider_event_id'])->exists()) { $counts['duplicate']++; return; }
        DB::transaction(function () use ($device,$runId,$event,&$counts) {
            $occurred=CarbonImmutable::parse($event['occurred_at']);$date=$occurred->setTimezone($event['source_timezone'])->toDateString();
            ...
            $mapping=$mappings->count()===1?$mappings->first():null;$reason=empty($event['provider_person_id'])?'person_missing':($mappings->isEmpty()?'person_unmapped':($mappings->count()>1?'person_mapping_ambiguous':null));
