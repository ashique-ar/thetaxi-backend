...
class CorporateContractualDistanceCalculator
{
    ...
    public function calculate(string $corporateId, string $serviceTypeId, array $journeyPoints): ?array
    {
        ...
        if ($rateMethod === 'normal_rate')
            $totalDistance += $movementBillable;
        ...
        return [
            'pickup_distance' => round($outboundBillable, 2),
            'origin_to_pickup_distance' => round($outboundActual, 2),
            'journey_distance' => round($journeyDistance, 2),
            'delivery_distance' => round($returnBillable, 2),
            'dropoff_to_return_distance' => round($returnActual, 2),
            'total_distance' => round($totalDistance, 2),
            'total_billable_distance' => round($totalDistance, 2),
            'journey_duration_seconds' => (int) $passengerLegs->sum('duration_seconds'),
            'total_duration_seconds' => (int) collect($legs)->sum('duration_seconds'),
            ...
            'distance_policy' => [
                ...
                'route_contract_version' => $routeContract['version'],
                'route_template' => $routeContract['template'],
                'resolved_contractual_anchors' => array_map(fn($anchor) => $this->snapshotAnchor($anchor), $anchors),
                'resolved_contractual_legs' => $legs,
                'defined_origin' => $this->snapshotAnchor($anchors[0]),
                'defined_return' => $this->snapshotAnchor($anchors[array_key_last($anchors)]),
                'include_origin_to_pickup' => (bool) $policy->include_origin_to_pickup,
                'include_dropoff_to_return' => (bool) $policy->include_dropoff_to_return,
    ...
    private function route(array $from, array $to, string $leg): array
    {
        if ($this->sameCoordinates($from, $to))
            return ['distance_km' => 0.0, 'duration_seconds' => 0, 'reference' => null];
        $route = $this->maps->distanceAndDuration($from, $to);
        if (!isset($route['distance_km'], $route['duration_seconds']) || ((float) $route['distance_km'] <= 0 && (int) $route['duration_seconds'] <= 0))
    ...
    private function snapshotAnchor(array $anchor): array
    {
        return collect($anchor)->only(['sequence', 'type', 'owner_type', 'owner_id', 'location_id', 'label', 'address', 'latitude', 'longitude'])->all();
    }

    private function legLabel(array $from, array $to, int $index): string
    {
        if ($index === 0 && $to['type'] === 'booked_pickup')
            return 'defined origin to passenger pickup';
        if ($from['type'] === 'booked_dropoff' && !in_array($to['type'], ['booked_stop', 'booked_dropoff'], true))
            return 'passenger drop-off to defined return';
        if (in_array($from['type'], ['booked_pickup', 'booked_stop'], true) && in_array($to['type'], ['booked_stop', 'booked_dropoff'], true))
            return 'passenger journey';
    ...
    {
        return abs((float) $from['latitude'] - (float) $to['latitude']) < 0.0000001 && abs((float) $from['longitude'] - (float) $to['longitude']) < 0.0000001;
    }
}
