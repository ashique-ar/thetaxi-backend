...
Hidden/empty adjacency is now explicit for every one of those conditional presenters. The booking desk overlaps the hero, so whichever managed section is the first rendered sibling receives the same 54px clearance: partner register, fleet, services, destinations, packages, offers, proof, testimonials, stories, or FAQ. Empty sections still emit no placeholder markup, all original Blade conditions remain independent, and no later section inherits the first-sibling adjustment. This covers sparse homepage combinations without changing CMS/settings ownership or fabricating content.
